...
eos = IC(end); % IC contains the transitions throughout the states.
% Find all states with multiple unique transitions. These are the only
% states that would need to be arithmetically encoded. A state with only
% one transition needs no arithmetic code.
mon = [];
for i=1:numel(IA)
    icf = find(IC==i);
    if numel(icf) > 1
...
N1 = size(C,1); S1 = sparse(N1,N1)+1;E1 = sparse(numel(IA),numel(HC))+1; % Markov States, Making the Transition Matrix. All transitions/emissions get one weight.
emlist = HIC(2:numel(HIC)-1); % Emission List.
for i=1:L-k
    S1(IC(i),IC(i+1)) = S1(IC(i),IC(i+1)) + 100; % Large weight to transitions that are present.
    linu = emlist(i);
    E1(IC(i),linu) = E1(IC(i),linu) + 100; % Large weight to emissions that are present.
end

esttr = S1./(eps+sum(S1,2)); % Estimate of the transition probabilities.
estemit = E1./sum(E1,2); % Estimate of the emission probabilities.

y = {}; % States we hit.
sec = zeros(1,numel(mon)); % State Entropy vector (for each multi-transmission state)
for i=1:numel(mon)
...
% Finding the stationary probability of states.
[V,D] = eigs(esttr',6);
D=diag(D);
Da = abs(D);
...
for i=1:numel(mon)
    siq = mon(i);sq = ceil(10000*trref(siq,:))+1;sq2 = trref(siq,:)+0.0001;sq2 = sq2/sum(sq2); % Give every output a possibility, in case the hmmtrain broke.
    icf = find(IC==siq);
    if siq==eos
